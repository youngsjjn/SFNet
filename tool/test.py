...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/cityscapes/cityscapes_gateseg101_aspp8170.yaml', help='config file')
    # parser.add_argument('--config', type=str, default='config/rugd/rugd_pspnet50.yaml', help='config file')
...
def main():
    ...
    if not args.has_prediction:
        if args.arch == 'sfnet':
            ...
            model = SFNet(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor,
                          output_stride=args.output_stride, pretrained=False)
        elif args.arch == 'sfnet_mobile':
            from model.sfnet_mobile import SFNet_mobile
            model = SFNet_mobile(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor,
                                 output_stride=args.output_stride, pretrained=False)

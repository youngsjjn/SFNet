...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/cityscapes/cityscapes_gateseg101_asppno1.yaml', help='config file')
    # parser.add_argument('--config', type=str, default='config/ctx/ctx_gateseg101_aspp.yaml', help='config file')
...
def check(args):
    ...
    elif 'gate' in args.arch:
        assert (args.train_h - 1) % 8 == 0 and (args.train_w - 1) % 8 == 0
    elif 'aspp' == args.arch:
        assert (args.train_h - 1) % 8 == 0 and (args.train_w - 1) % 8 == 0
    elif 'annn' in args.arch:
        assert (args.train_h - 1) % 8 == 0 and (args.train_w - 1) % 8 == 0
    else:
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'sfnet':
        ...
        model = SFNet(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, criterion=criterion,
                        output_stride=args.output_stride, BatchNorm=BatchNorm)
        modules_ori = [model.layer0, model.layer1, model.layer2, model.layer3, model.layer4]
        modules_new = [model.aspp, model.cls, model.aux, model.gate3, model.gate2, model.gate1,
                       model.conv_f3, model.conv_f2, model.conv_h3, model.conv_h2]

    elif args.arch == 'sfnet_mobile':
        from model.sfnet_mobile import SFNet_mobile
        model = SFNet_mobile(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, criterion=criterion,
                        output_stride=args.output_stride, BatchNorm=BatchNorm)
